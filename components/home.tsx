...
const HomeBanner: React.FC = () => {
  ...
  const items = [
    {
      name: 'Gourmet Dishes',
      description: 'Experience the best gourmet dishes crafted by our top chefs.',
    ...
    {
      name: 'Wine Selection',
      description: 'Choose from a wide variety of wines to complement your meal.',
  ...
  return (
    <div className="relative h-screen overflow-hidden">
      {items.map((item, index) => (
        ...
        >
          <div className="absolute inset-0 bg-black bg-opacity-50 flex flex-col justify-center items-center text-center text-white p-4">
            ...
            <button className="mt-8 px-6 py-3 bg-yellow-500 text-black font-semibold rounded-lg shadow-lg hover:bg-yellow-600 transition duration-300">
              See More
            </button>
          </div>
        </div>
      ...
      <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 flex space-x-4">
        <button className="p-2 bg-gray-300 rounded-full hover:bg-gray-400 transition duration-300" onClick={prevSlide}>
          <ArrowLeft className="text-black" />
        </button>
        <button className="p-2 bg-gray-300 rounded-full hover:bg-gray-400 transition duration-300" onClick={nextSlide}>
